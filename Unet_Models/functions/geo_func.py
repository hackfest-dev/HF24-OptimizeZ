...
def geoParams(image_path):
    from keras.models import load_model
    model = load_model('Unet_Models/functions/woundSegmentation_model.h5')
    IMG_WIDTH = 128
    IMG_HEIGHT = 128
    IMG_CHANNELS = 3
    ...
    cv2.imwrite(os.path.join(output_directory,"woundColorIsolation.png"), image)
    ...
    cv2.imwrite(os.path.join(output_directory,"GreyScale_text.png"), image)
